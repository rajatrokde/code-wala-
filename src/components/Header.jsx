...
export default function Header({ 
  ...
  // Simulate active devs fluctuating
  useEffect(() => {
  ...
  return (
    <header className="w-full px-6 py-4 flex items-center justify-between z-30 select-none">
      {/* Left: Clock */}
      <div className="flex items-center gap-2 glass-pill px-3.5 py-1.5 rounded-full text-xs font-mono text-zinc-300 tracking-wider">
        <Clock className="w-3.5 h-3.5 text-emerald-400" />
        <span>{timeStr || '14:24 PM'}</span>
      </div>

      {/* Center: Live Active Devs Badge (Matching hornokplease badge) */}
      <div className="flex items-center gap-2 glass-pill px-4 py-1.5 rounded-full text-xs text-zinc-200 shadow-lg">
        ...
        <span className="text-zinc-400 lowercase">devs on the highway</span>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="glass-pill p-2 rounded-full text-zinc-300 hover:text-white transition-all cursor-pointer hover:scale-105"
        >
        ...
        <button
          ...
          className={`glass-pill p-2 rounded-full transition-all cursor-pointer hover:scale-105 ${
